...
DragonBonesCSharp 是 DragonBones 组织维护的 C# runtime 的 fork。上游 README 将其描述为 DragonBones C# Runtime，包含 common library 与 Unity 接入。Future2D 的副本用于参考，不会把 DragonBones 数据格式或原始实现的所有权转移给 Future2D。

## 适配范围

该仓库面向 **DragonBones 骨骼动画数据** 及其 C# 运行时使用，并提供文档化的 Unity 集成。它代表了托管运行时中的 2D 骨骼动画路径：应用需要加载模型和动画数据、求值动画状态，并将结果姿态传给引擎集成。

该 fork 不代表支持 Spine、Live2D 或 Future2D 包。DragonBones 兼容性仅针对上游定义的数据和运行时契约。
...
该 fork 用于托管运行时和引擎集成研究，不表示 Future2D 一致性、转换路径、分支同步或支持承诺。评估代码行为时，应查看精确的上游分支和仓库历史。
