...
本仓库 fork 自 Esoteric Software 的 Spine Runtimes。上游项目为 Spine 这套 2D 骨骼动画工具提供加载和求值运行时。Future2D 保存副本用于研究，Spine、其文件格式、运行时行为、商标和许可证仍归 Esoteric Software。

## 适配范围

上游运行时用于让不同游戏或应用工具链使用 **Spine 骨骼动画数据**。Spine 数据除纹理外，还涉及骨骼结构、attachments、skins、时间线、绘制顺序、约束及其他运行时语义。兼容性测试需要检查这些关系及其求值结果。
...
该 fork 用于格式和运行时研究。实际补丁、分支差异和同步状态应在对应 fork 与上游分支的比较中确认。
